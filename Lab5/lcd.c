 ...
 * @date 1/24/2019
 *
 * @param dispText  Text in string it takes in to rotate
 */
void lcd_rotatingBanner(char dispText[]){
    ...
    //It is recommended that you use only lcd_init(), lcd_printf(), lcd_putc, and lcd_puts from lcd.h.
           ...
           //Initialize the LCD
           lcd_init();
           //Size of the string to be displayed plus the 21 spaces
           int size_of_string = strlen(dispText) + 21;
           //Char array that will hold the dispText plus the spaces
           char space_plus_dispText[100] = "                    ";
           //Initialize the incrementing variables
           int i = 0;
           int j = 0;
           //Char array that will hold the string that needs to be printed at the moment
           char array_to_print[100] = "";
           //Copies the dispText char array into the space_plus_dispText char array
           strcat(space_plus_dispText,dispText);

           //Rotate through the size of the text char array plus the spaces
           for(i = 0; i < size_of_string; i++){
               //Set the print array to 20 characters at a time
             for(j = 0; j < 20; j++){
                 array_to_print[j] = space_plus_dispText[i+j];
              }
              //Print the array 20 characters at a time
              lcd_printf(array_to_print);
              //Wait .3 seconds
              timer_waitMillis(300);
              //Once the whole char array has been displayed, restart the for loop
